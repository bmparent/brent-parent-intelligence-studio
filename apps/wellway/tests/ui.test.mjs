...
await tick();
assert.equal(button("Approve plan").disabled, true);
value(
...
assert.match(d.querySelector(".conversation").textContent, /not its cause/);
assert.match(d.querySelector(".conversation").textContent, /No live model/);
await click("What is in my history?");
assert.match(
  d.querySelector(".conversation").textContent,
  /unverified context/,
);
assert.match(d.querySelector(".conversation").textContent, /history-2023-knee/);
d.querySelector('[aria-label="Close dialog"]').click();
await tick();
...
await click("Join visit");
assert.match(
  d.querySelector("[role=dialog]").textContent,
  /Camera and microphone are simulated/,
);
await click("Join visit");
await click("My sleep");
assert.match(d.querySelector(".visit-caption").textContent, /7 days/);
await click("Hide camera");
assert.ok(d.querySelector(".camera-off"));
await click("Unmute");
assert.match(d.querySelector(".member-pip").textContent, /Simulated mic on/);
await click("Hide captions");
assert.equal(d.querySelector(".visit-caption"), null);
await click("Show captions");
await click("End visit");
...
await click("Join visit");
for (const input of d.querySelectorAll(".visit-scope input[type=checkbox]")) {
  input.click();
  await tick();
}
await click("Join visit");
