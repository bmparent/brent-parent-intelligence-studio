...
export function Plan({ checkin }: { checkin: () => void }) {
  ...
  return (
    <>
      ...
      {state.reviews.some(
        (r) => r.status === "approved" && r.targetPlanId === "followup",
      ) && (
        <section className="panel approved-followups">
          <h2>Agreed follow-up</h2>
          {state.reviews
            .filter(
              (r) => r.status === "approved" && r.targetPlanId === "followup",
            )
            .map((r) => (
              <article key={r.id}>
                ...
                <p className="answer-text">{r.proposal}</p>
              </article>
            ))}
        </section>
      )}
      {editing && (
        <Modal
          ...
          subtitle="Adjust this activity in your plan."
          onClose={() => setEditing(null)}
          dirty={
            JSON.stringify(editing) !==
            JSON.stringify(state.plan.find((p) => p.id === editing.id))
          }
        >
          <form
            onSubmit={(e) => {
