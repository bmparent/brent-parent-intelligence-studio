...
export function SiteGallery({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section
      id="site-gallery"
      className="ew-site-gallery ew-shell"
    ...
    >
      <div className="ew-section-heading">
        <div>
          ...
          <p className="ew-gallery-intro">
            Games, tools, storefronts, and experiments. A growing gallery of the
            sites we’ve made.
          </p>
        </div>
        {compact && (
          <a className="ew-text-link" href="/work#site-gallery">
      ...
      {!compact && (
        <div className="ew-gallery-controls">
          <div
            className="ew-gallery-filters"
            role="group"
            aria-label="Filter site gallery"
          >
          ...
          <label className="ew-gallery-search">
            Search sites
            <input
              type="search"
              value={query}
      ...
      {!compact && (
        <p className="ew-gallery-count" role="status">
        ...
        {visible.map((site) => (
          ...
          >
            <span className="ew-site-window">
              <span className="ew-site-browser" aria-hidden="true">
                <i />
                <i />
                <i />
              </span>
              <img
                className={
                  site.previewLabel ? 'ew-site-reference-image' : undefined
                }
                src={site.thumbnail}
                ...
                loading="lazy"
              />
              <span className="ew-site-view" aria-hidden="true">
                View preview ↗
              </span>
            </span>
            <span className="ew-site-category">{site.category}</span>
            <span className="ew-site-title">
              {site.title}
      ...
      {visible.length === 0 && (
        <div className="ew-gallery-empty">
      ...
      >
        {selected && (
          <div className="ew-site-dialog-content">
            <div className="ew-site-dialog-heading">
              <p className="ew-eyebrow">{selected.category} / Site preview</p>
              <button
                type="button"
            ...
            <img
              className="ew-site-full-image"
              src={selected.image}
              alt={`${selected.title} homepage`}
              width="1440"
              height="1100"
            />
            <div className="ew-site-dialog-copy">
              <h3 id="site-preview-title">{selected.title}</h3>
              ...
              {selected.url ? (
                ...
                <span className="ew-site-private">
                  {selected.previewLabel || 'Private project · Design preview'}
                </span>
              )}
            </div>
          </div>
