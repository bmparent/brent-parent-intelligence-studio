...
export function MediaBrand({project:p,edit,guard,notify,close}:{project:Project;edit:(p:Project|((p:Project)=>Project),group?:string)=>void;guard:()=>()=>boolean;notify:(s:string)=>void;close:()=>void}) {
 const [placement,setPlacement]=useState<SectionType>('hero'),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 const s=p.sections.find(v=>v.id===placement)!,m=s.media||defaultMedia();
 ...
 function media(patch:Partial<MediaSettings>) {edit({...p,sections:p.sections.map(v=>v.id===placement?{...v,media:{...m,...patch}}:v)},'media-'+placement);}
 ...
 async function upload(file?:File) {
  ...
  try {
   ...
   const next={...p,sections:p.sections.map(v=>v.id===placement?{...v,image:data,media:{...defaultMedia(),fit:placement==='header'?'contain' as const:'cover' as const,sourceId,sourceName:file.name.slice(0,120)}}:v)};
 ...
 <div className="pg-field"><label htmlFor="pg-media-placement">Image placement</label><select id="pg-media-placement" value={placement} onChange={e=>setPlacement(e.target.value as SectionType)}>{p.sections.filter(v=>v.id!=='footer').map(v=><option key={v.id} value={v.id}>{labels[v.id]} · {v.id==='header'?'logo':v.id==='hero'||v.id==='work'?'image':'background'}</option>)}</select></div>
