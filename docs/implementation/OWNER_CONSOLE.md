...
| Works owner operations tables | Manual work, checkpoints, action audit via additive `0007_operations.sql` | Same validation Turso binding; owner JWT plus action checks | On action; append-only audit, no automatic deletion | Medium; reasons must avoid customer secrets | Unavailable until migration; apply only to verified validation database |
