...
6. Set `EIDOS_MAINTENANCE_TOKEN` and `EIDOS_PLATFORM_TOKEN` in the Eidos Works GitHub repository to the same independent values used in Vercel. The hourly workflow calls the authenticated Sentinel maintenance endpoint directly. It requires both credentials and fails visibly when either is missing. Scheduled workflows run from the default branch. The first production run through the Pages URL returned 403 from a GitHub runner while the same authenticated request passed locally; direct backend scheduling retains application authentication without depending on the website request path.
