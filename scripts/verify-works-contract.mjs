...
for (const path of paths) {
  ...
  // The exporter deliberately canonicalizes the baseline SQL to UTF-8 LF.
  const normalization=path==='migrations/0001_eidos_platform.sql'?'UTF-8 LF':'exact bytes';
  if(normalization==='UTF-8 LF') source=Buffer.from(source.toString('utf8').replace(/\r\n/g,'\n'));
  const vendor = await readFile(resolve(backend, 'apps/sentinel-lab/lib/works/vendor', path));
...
await writeFile('artifacts/implementation/2026-09-21/compatibility.json', JSON.stringify(receipt, null, 2));
