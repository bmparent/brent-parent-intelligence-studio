...
const siteUrl = (
  process.env.VITE_SITE_URL || 'https://eidos-works.com'
).replace(/\/+$/, '');
...
if (!Array.isArray(rawArticles))
  throw new Error('src/data/articles.json must contain an array.');

for (const article of rawArticles) {
  if (!article || typeof article !== 'object' || Array.isArray(article))
    throw new Error('Every article must be an object.');
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(article.slug ?? ''))
    throw new Error('Article slugs must be lowercase kebab-case.');
  if (article.canonicalPath !== `/insights/${article.slug}`)
    throw new Error(`Invalid canonical path for ${article.slug}.`);
  if (article.ogImage !== `/insights-og/${article.slug}.svg`)
    throw new Error(`Invalid OG image path for ${article.slug}.`);
...
function generateSitemap() {
  const staticPages = [
    ...
    {
      loc: absolute('/community/agent-guide'),
      lastmod: '2026-09-05',
      priority: '0.7',
    },
    {
      loc: absolute('/community/guidelines'),
      lastmod: '2026-09-05',
      priority: '0.7',
    },
    { loc: absolute('/privacy'), lastmod: '2026-09-05', priority: '0.7' },
    { loc: absolute('/terms'), lastmod: '2026-09-05', priority: '0.7' },
    {
      loc: absolute('/shop/cinematic-starter'),
      lastmod: '2026-09-05',
      priority: '0.7',
    },
    ...
    {
      loc: absolute('/work/production-dashboard'),
      lastmod: '2026-07-11',
      priority: '0.8',
    },
    {
      loc: absolute('/work/storefront-experience'),
      lastmod: '2026-07-11',
      priority: '0.8',
    },
  ...
  const urls = [...staticPages, ...articlePages]
    .map(
      (entry) =>
        `  <url><loc>${xmlEscape(entry.loc)}</loc><lastmod>${entry.lastmod}</lastmod><priority>${entry.priority}</priority></url>`,
    )
    .join('\n');
...
function generateOgSvg(article) {
  ...
  const titleSvg = titleLines
    .map(
      (line, index) =>
        `<text x="72" y="${238 + index * 58}" class="title">${svgEscape(line)}</text>`,
    )
    .join('\n  ');
...
const expectedOgFiles = new Set(
  articles.map((article) => `${article.slug}.svg`),
);
for (const entry of await readdir(ogDir, { withFileTypes: true })) {
  if (
    entry.isFile() &&
    entry.name.endsWith('.svg') &&
    !expectedOgFiles.has(entry.name)
  ) {
...
for (const article of articles) {
  ...
  if (!target.startsWith(`${resolve(ogDir)}${sep}`))
    throw new Error(`Unsafe OG image target for ${article.slug}.`);
...
await writeFile(
  resolve(reportDir, 'latest-generation-report.json'),
