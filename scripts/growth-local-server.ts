...
const env={EIDOS_GROWTH_DB:db,EIDOS_PLATFORM_TOKEN:'local-controlled-only-secret-0000000000',EIDOS_GROWTH_OWNER_TOKEN:'local-controlled-owner-secret-0000000000',EIDOS_INQUIRY_MAILER:{fetch:(async()=>Response.json({ok:true,receipt:crypto.randomUUID()})) as typeof fetch}};
...
createServer(async(req,res)=>{
  try {
    ...
    else if(url.pathname==='/api/project-inquiries') response=req.method==='POST'?await inquiry({request,env}):Response.json({deliveryConfigured:true,privateMailerConfigured:true});
    else if(url.pathname==='/api/public-config') response=Response.json({gaMeasurementId:'G-8N7Y7EM4CS',accountsReady:false,assistantReady:false,aiReady:false,shopReady:false});
    else {
